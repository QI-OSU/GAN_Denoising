...
class Unet(nn.Module):
    ...
    def __init__(self, im_channels, model_config):
        ...
        for i in range(len(self.down_channels) - 1):
            self.downs.append(DownBlock(self.down_channels[i], self.down_channels[i + 1], t_emb_dim=None,
        ...
        for i in range(len(self.mid_channels) - 1):
            self.mids.append(MidBlock(self.mid_channels[i], self.mid_channels[i + 1], t_emb_dim=None,
                                      num_heads=self.num_heads,
        ...
        for i in reversed(range(len(self.down_channels) - 1)):
            self.ups.append(UpBlockUnet(self.down_channels[i] * 2, self.down_channels[i - 1] if i != 0 else self.conv_out_channels,
                                        t_emb_dim=None, up_sample=self.down_sample[i],
                                        num_heads=self.num_heads,
